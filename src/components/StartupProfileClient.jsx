...
import { MapPin, ArrowLeft, Bookmark, BookmarkCheck, Share2, ThumbsDown, Rocket } from 'lucide-react'
...
export default function StartupProfileClient({ startup }) {
  ...
  const handleSupport = async () => {
    ...
    try { await recordSwipe({ startup_id: startup.id, direction: 'right' }); setShowCommunity(true) } catch {}
  }

  const handleSave = async () => {
    try {
      if (saved) { await unsaveStartup(startup.id); setSaved(false); toast.success('Removed from saved') }
      else { await saveStartup(startup.id); setSaved(true); toast.success('🔖 Saved!') }
  ...
  return (
    <div className="max-w-2xl mx-auto px-5 py-10">
      <button onClick={() => router.back()} className="flex items-center gap-2 text-muted hover:text-ink transition-colors mb-8 text-sm font-medium">
      ...
      <div className="bg-white rounded-3xl border border-border shadow-card overflow-hidden mb-5">
        <div className="p-8 border-b border-border">
          <div className="flex items-start gap-5">
            <div className="w-20 h-20 rounded-2xl flex items-center justify-center text-4xl flex-shrink-0 bg-gray-50">
              {startup.logo_url
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap gap-2">
                <span className="text-xs font-semibold px-3 py-1.5 rounded-full bg-purple-100 text-purple-700">{startup.category}</span>
                {startup.city && (
                ...
                {startup.stage && (
                  <span className="text-xs font-semibold px-3 py-1.5 rounded-full bg-coral-50 text-coral">{startup.stage}</span>
                )}
              </div>
            </div>
          </div>
        ...
        <div className="flex divide-x divide-border">
          <div className="flex-1 text-center py-4">
            ...
            <div className="text-xs text-muted mt-0.5">Supporters</div>
          </div>
          <div className="flex-1 text-center py-4">
            <div className="font-display text-2xl font-bold text-ink">{startup.stage || '—'}</div>
            <div className="text-xs text-muted mt-0.5">Stage</div>
          </div>
          <div className="flex-1 text-center py-4">
            <div className="font-display text-2xl font-bold text-ink">{channels.length}</div>
            <div className="text-xs text-muted mt-0.5">Channels</div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl border border-border shadow-card p-7 mb-5">
        <h2 className="font-display text-xl font-semibold mb-5">About {startup.name}</h2>
        <div className="space-y-5">
          ...
          {startup.target_customer && (
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-muted mb-2">Who it's for</p>
      ...
      {startup.founder_name && (
        <div className="bg-white rounded-3xl border border-border shadow-card p-7 mb-5">
          ...
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-full bg-coral flex items-center justify-center text-white font-bold text-lg flex-shrink-0">
            ...
            {channels.map(({ key, url }) => {
              ...
              return (
                <a key={key} href={url} target="_blank" rel="noopener noreferrer"
                  ...
                  <div className="w-9 h-9 rounded-xl bg-white flex items-center justify-center text-lg shadow-sm">{meta.icon}</div>
                  <span className={`text-sm font-semibold ${meta.text}`}>{meta.label}</span>
                  <span className={`ml-auto text-sm ${meta.text} opacity-40 group-hover:opacity-100`}>→</span>
                </a>
              )
            })}
          </div>
      ...
      {/* Share CTA */}
      <div className="bg-gradient-to-br from-coral-50 to-purple-50 rounded-3xl border border-border p-7 text-center">
        <p className="font-display text-xl font-semibold mb-2">Know someone who'd love this?</p>
        <p className="text-muted text-sm mb-5">Share {startup.name} with your network and help them grow.</p>
        <button onClick={() => setShowShare(true)}
          className="bg-coral text-white font-semibold px-6 py-3 rounded-full shadow-coral hover:bg-coral-600 transition-all inline-flex items-center gap-2">
